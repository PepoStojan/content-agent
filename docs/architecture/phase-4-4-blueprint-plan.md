...
The four previously-open decisions from §0, §11, and §13 are now resolved and locked. BD5 (below) resolves the one remaining open question this document had left explicitly unresolved (§11/§18: "Manual (non-AI) node editing remains a separate, unresolved implementation-time question"). Nothing below is subject to further relitigation absent a demonstrated need.
...
| BD5 | **Manual Blueprint editing — approved, resolving §11/§18's open question (2026-08-20).** Users may directly edit, per node: `title`, `goal`, `research_support`, `unique_contribution`, `entities`, `internal_link_targets`, `evidence_requirement`, `writing_notes`, `target_word_count`. **No structural tree changes** in this phase — no add/delete node, no reorder, no parent/child re-assignment; only existing leaf/structural nodes' own fields may change. AI per-node regeneration remains rejected (BD2, unchanged) and AI Blueprint regeneration remains whole-document only (unchanged) — a manual edit is a distinct, non-AI action. Versioning: editing any node creates a **new `blueprint_versions` row with a full new `blueprint_nodes` snapshot** (the exact same whole-document-versioning shape AI regeneration already uses, generalized to a second trigger) — never an in-place mutation of an existing version's nodes. The previous Blueprint version remains fully immutable and permanently retrievable, same guarantee as every other versioned artifact in this schema. The new version starts at `status='draft'` and requires its own separate Approve action — a manual edit is never auto-approved. `brief_version_id` is carried forward unchanged (the exact same source Brief the edited version was already built from — a manual field edit is not a new Brief synthesis and must not silently re-pin lineage). Existing `content_documents`/`content_versions` remain tied to the *original* Blueprint version's node ids and are never touched by a Blueprint edit (same "no silent cascade" principle as every cross-stage regeneration in this schema, §9). Only once the new, edited Blueprint version is itself approved does it become the source for *future* Content generation — approving it does not retroactively re-point or invalidate content already generated against the prior version. Manual edits are visible in Blueprint version history exactly like AI regenerations — no separate "edit log" or diff view is introduced; the new version row itself, with its own `created_at`/`created_by`, is the record. | **LOCKED** |
| BD6 | **No automatic Content carry-forward across Blueprint versions — confirmed, not a gap (2026-08-20).** When any new `blueprint_versions` row is created (BD5 manual edit or AI regeneration, BD2), every leaf node gets a fresh id (BD5's own requirement), so **all** existing Content for that Blueprint — not just the edited section's — becomes structurally orphaned from the new current version: it stays permanently, correctly attached to its original (now non-current) Blueprint version, and the new version starts with zero Content anywhere. This is deliberate, matches the "no silent cascade" principle already governing every other cross-stage regeneration in this schema, and is the safest default for V1 — see the dedicated analysis section below (§19). What is **not** yet addressed: the *UI* never explains this to a user, so a single-field Blueprint edit today silently makes the entire Content Editor look empty with no stated reason — a real, confirmed product-communication gap (found via a live QA run, 2026-08-20), flagged for future UI work, not resolved by this decision. | **LOCKED** | §19 |
...
- **Manual node editing is a separate, distinct concern and is unaffected by BD2 — now resolved and locked as BD5 (2026-08-20), no longer an open implementation-time question.** Where Design V1 already supports directly editing an individual node's fields by hand (no model call involved — the user typing a different `title`/`goal`/etc. directly into the inspector panel), that is now approved: field-level edits only (the 9 fields listed in BD5), no structural tree changes. BD2 only ever rejected an *AI-driven* per-node "Regenerate" action, not human editing of a node's own fields — that distinction holds unchanged. Versioning is resolved as **not** an in-place update: every manual edit creates a new `blueprint_versions` row with a full new `blueprint_nodes` snapshot, starting at `draft`, requiring its own separate approval — the same whole-document-versioning shape AI regeneration already uses (BD5's full detail).
...
- Per-node AI Blueprint regeneration (§11, BD2) — locked as rejected, not built. Manual (non-AI) node editing is **no longer** an open question — approved and locked as BD5 (2026-08-20): field-level only, no structural tree changes (no add/delete/reorder/re-parent node), versioned as a new whole-Blueprint-version snapshot, same as AI regeneration.
- Structural tree changes via manual editing (add node, delete node, reorder, parent/child re-assignment) — explicitly out of scope for BD5 (2026-08-20). A future decision, not implied or opened by BD5's field-level approval.
...
## 19. BD6 — the Content-orphaning consequence of BD5, analyzed (2026-08-20)

Found empirically, not hypothesized: the first real Phase 4.6 QA execution against a live BD5-edited project (project `64f3a6aa-3090-457a-ae54-3a1d506a87d2`) correctly reported "No leaf sections have generated content yet" for a Blueprint that, one version earlier, had two real, human-approved-adjacent Content sections. A single-field edit to *one* node (a title typo-style fix) had orphaned *all 22* leaf sections' Content standing, not just the edited one — because BD5 always clones the entire node tree with fresh ids (§0's rationale, unchanged). This section is the analysis this task was asked to produce before any implementation is proposed.

**1. What should the user see after editing an approved Blueprint?** Today: nothing informative. The Content Editor tab re-renders against the new version's leaf nodes, all of which show the ordinary "not generated" empty state — visually identical to a brand-new project that has never had Content generated at all. There is no signal that this project *did* have Content, or that an edit is what caused the change. This is the sharpest real UX gap this analysis found.

**2. Should the UI explicitly say "This Blueprint version has no Content yet"?** Yes, in spirit — but more precisely, it should say something that distinguishes *this* case (a version that superseded one that had Content) from a first-generation project. A bare "no Content yet" message is honest but incomplete; a user who just clicked Save on a Blueprint edit and now sees 22 empty sections needs to understand *why*, not just *that*. Not designed further here (no implementation proposed), but the requirement is more specific than the question's literal wording suggests.

**3. Should the system offer "Generate Content for this Blueprint"?** A per-section "Generate section" action already exists (Phase 4.5). A whole-document convenience wrapper ("Generate All Sections," looping over every leaf) was explicitly flagged as legitimate future UX in Phase 4.5 §10/§21 but deliberately not built — this finding raises the priority of that backlog item (it would meaningfully soften BD6's UX gap) but does not change its scope or design.

**4. Should old Content remain visibly accessible under the old Blueprint version?** It remains fully, correctly *stored* (BD5 point 8 — nothing is deleted or mutated). It is **not visibly accessible** through any UI today: the Content Editor has no concept of "view Content as it stood under Blueprint version N," only "view Content for the current Blueprint version." This is a second real, confirmed gap — old Content is safe but effectively invisible after an edit, which from a user's perspective is indistinguishable from data loss unless they already understand the versioning model deeply enough to know to check Blueprint version history.

**5. What happens if only one node was manually edited?** Confirmed by direct DB inspection during the live QA run: editing one node's `title` field orphaned all 22 leaf sections' Content standing simultaneously, because every node — not just the edited one — receives a new id in the new version (BD5's own requirement, §0's "no shared rows across versions" rule). The blast radius of a single-field edit is the entire document's Content, not the edited section alone. This is disproportionate to what a user would reasonably expect from "I fixed a typo in one section's goal," and is the single most important finding of this analysis.

**6. Should unchanged sections ever be automatically carried forward?** **No — rejected outright for V1, not deferred.** Reasoning:
   - **No node-identity concept exists to carry forward safely.** `blueprint_nodes.id` is opaque and version-scoped; nothing in the schema says "this new node succeeds that old node." Any carry-forward logic would need to invent a matching heuristic (position? title similarity?) that is itself a source of silent, hard-to-audit error — matching the wrong old section to the wrong new node would be worse than the current honest-empty state.
   - **It would conflict with CD5's clean provenance signal.** Content's authorship model is exactly two states today: AI-generated (`generation_run_id` present) or manually edited (`null`). A "carried forward, unchanged" row is neither — it would need a third provenance concept, weakening the "absence of generation_run_id is itself the honest signal" design Phase 4.5 §11 deliberately chose over adding a status column.
   - **It breaks the "no silent cascade" principle this entire schema relies on.** Every other cross-stage regeneration (Brief → Blueprint, Blueprint → Content) is already designed so a human decides whether staleness matters (Phase 4.4 §9, Phase 4.5 §15) — carrying Content forward automatically here would be the one exception, inconsistent with the rest of the pipeline's own stated discipline.
   - **It can silently misrepresent grounding.** If the edited node's `entities`/`evidence_requirement`/`target_word_count` changed as part of the same edit (a legitimate reason to edit a node at all), auto-carried-forward Content for *that* node would no longer match its own node's directive — a worse failure mode than an honest "not generated," because it looks complete while being subtly wrong.

**7. Risks of automatic carry-forward, ranked:** (a) false continuity — a user believes nothing changed and never re-checks content that is now structurally mismatched to its own node; (b) no safe node-matching mechanism, especially once structural editing (explicitly deferred, §18) eventually exists and node counts/order can genuinely change; (c) provenance-model complexity (a third authorship state); (d) QA interaction risk — carried-forward Content could reappear as "current" without a fresh QA run ever having evaluated it against the *edited* node's fields, undermining QD6's whole purpose; (e) precedent inconsistency with every other stage's "no silent cascade" rule.

**8. Does the current BD5 design remain the safest V1 choice?** **Yes, confirmed, not merely re-affirmed by default.** The versioning/orphaning behavior itself (no auto-carry-forward) is correct and should not change — §19.6/§19.7 above is a genuine re-examination, not a rubber stamp, and it concludes the current design is right. What must change is **communication, not architecture**: the gaps are in §19.1–§19.4 (UI never explains what happened, no way to view old Content under an old Blueprint version), not in the underlying data model or versioning discipline. BD6 (the locked-decisions table entry above) records this precisely: the *behavior* is locked as correct; the *UI gap* is flagged as a real, confirmed backlog item, explicitly not solved by this analysis pass per this task's own "no implementation" constraint.

---

This document was originally written planning-only (no code, migrations, or API calls made producing it). That plan has since been fully implemented — see "Implementation status" near the top for what's actually shipped, migrated, and live-verified as of 2026-08-19. The one required schema change (§0, `blueprint_versions.brief_version_id`) has been applied via `20260824000001_phase4_4_blueprint_lineage.sql`, not merely flagged. BD5 (manual editing) and BD6 (Content-orphaning behavior, analyzed and confirmed correct) were locked 2026-08-20, the latter following a real, live QA execution that surfaced the exact consequence §19 analyzes.
